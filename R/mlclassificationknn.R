...
.knnClassificationManual <- function(dataset, options, formula, jaspResults){
  ...
  knn.fit <- kknn::kknn(formula = formula, train = train, test = test, k = options[['noOfNearestNeighbours']], 
                        distance = options[['distanceParameterManual']], kernel = options[['weights']],scale = options[['scaleEqualSD']])
  ...
  knn.train <- kknn::kknn(formula = formula, train = train, test = train, k = options[['noOfNearestNeighbours']], 
                        distance = options[['distanceParameterManual']], kernel = options[['weights']],scale = options[['scaleEqualSD']])
  res[["trainingError"]] <- round(length(which(knn.train$fitted.values != train[,.v(options[["target"]])])) / nrow(train), 3) 
  return(res)
}
...
.knnClassificationLoocv <- function(dataset, options, formula, jaspResults){
    ...
    res[["trainingError"]] <- round(length(which(predict(knn.fit, newdata = dataset) != dataset[,.v(options[["target"]])])) / nrow(dataset), 3)
...
.knnClassificationKfold <- function(dataset, options, formula, jaspResults){
    ...
    knn.train <- kknn::kknn(formula = formula, train = dataset, test = dataset, k = res[['optimal.k']], 
                          distance = res[["optimal.distance"]], kernel = res[['optimal.weights']],scale = options[['scaleEqualSD']])
                          print(knn.train)
    res[["trainingError"]] <- round(length(which(knn.train$fitted.values != dataset[,.v(options[["target"]])])) / nrow(dataset), 3) 
    return(res)
}
     ...
     if(is.null(jaspResults[["plotErrorVsK"]])){
       ...
       for(i in 1:options[["maxK"]]){
         ...
         knn.fit <- kknn::kknn(formula = formula,
                               ...
                               test = test,
                               k = i,
                               distance = res[["optimal.distance"]],
                               kernel = res[["optimal.weights"]],
